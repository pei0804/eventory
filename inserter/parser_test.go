	...
	p.RespByte = []byte(`
{
	...
	p.atdnJsonParse()
	if p.err != nil {
		t.Fatalf("[atdnJsonParse]Jsonの解析に失敗しました。%s", p.err)
	}
}
	...
	p.RespByte = []byte(`
{
	...
	p.connpassJsonParse()
	if p.err != nil {
		t.Fatalf("[connpassJsonParse]Jsonの解析に失敗しました。%s", p.err)
	}
}
...
[
	...
	p.doorkeeperJsonParse()
	if p.err != nil {
		t.Fatalf("[doorkeeperJsonParse]Jsonの解析に失敗しました。%s", p.err)
	}
}
